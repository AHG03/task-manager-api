...
def test_get_tasks_with_search_filter(client):
    ...
    response = client.get("/tasks?search=milk", headers=headers)
    assert response.status_code == 200
    tasks = response.json()
...
def test_get_tasks_with_search_no_results(client):
    ...
    response = client.get("/tasks?search=eggs", headers=headers)
    assert response.status_code == 200
    tasks = response.json()
    assert len(tasks) == 0
...
def test_get_tasks_with_search_and_completed_filter(client):
    ...
    client.put(
        f"/tasks/{task['id']}", json={"title": "Buy milk tomorrow", "completed": True}, headers=headers)

    response = client.get("/tasks?search=milk&completed=true", headers=headers)
...
def test_get_tasks_sort_by_title_ascending(client):
    ...
    response = client.get("/tasks?sort_by=title", headers=headers)
...
def test_get_tasks_sort_by_title_descending(client):
    ...
    response = client.get(
        "/tasks?sort_by=title&sort_order=desc", headers=headers)
...
def test_invalid_sort_field(client):
    ...
    response = client.get("/tasks?sort_by=random", headers=headers)
    assert response.status_code == 422
...
def test_invalid_sort_order(client):
    ...
    response = client.get(
        "/tasks?sort_by=title&sort_order=random", headers=headers
    )
...
def test_get_tasks_with_limit_and_offset(client):
    ...
    response = client.get("/tasks?limit=2&offset=1", headers=headers)
